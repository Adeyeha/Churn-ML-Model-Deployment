...
from starlette.status import HTTP_404_NOT_FOUND, HTTP_401_UNAUTHORIZED, \
        HTTP_503_SERVICE_UNAVAILABLE,HTTP_403_FORBIDDEN
from sqlalchemy.orm import Session

import db.crud,db.models, db.schemas
import db.database
...
@app.get(
    "/churn/{cust_ac_no}",
    tags=["churn"],
    response_model=db.schemas.Churn,
    summary="Gets a single Churn Result",
    response_description="Churn results for the provided Account Number",
    responses={404: {"model": None, "description": "Bank Account not found"}}
)
def get_churn(
        cust_ac_no: int = Path(
            ...,
    ...
    """
    Gets Churn Result For Account Number:

    - **Account Number**: The Account Number
    """
    res = db.crud.get_churn(session, cust_ac_no)

    if not res:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND, detail="Bank Account Not Found"
        )
